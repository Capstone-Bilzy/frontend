...
import com.android.bilzy.databinding.ItemManualReceiptBinding
import com.android.bilzy.domain.model.ReceiptItemDraft

/**
 * 직접 입력 화면 전용 인라인 편집 어댑터. item_ocr.xml/OcrItemAdapter와 같은 카드 형태를 따르되,
 * 품목명/수량/가격을 그 자리에서 바로 고칠 수 있어야 해서 읽기 전용 대신 EditText로 구성했다.
 * 편집 중 매 키 입력마다 리스트를 다시 그리면(EditText 포커스가 날아가) 타이핑이 끊기므로,
 * 항목 개수가 그대로인 업데이트(=글자 수정)는 notifyDataSetChanged를 생략한다.
 */
class ManualItemAdapter(
    private val onDelete: (Int) -> Unit,
    private val onChange: (Int, ReceiptItemDraft) -> Unit
) : RecyclerView.Adapter<ManualItemAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(val binding: ItemManualReceiptBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemManualReceiptBinding.inflate(LayoutInflater.from(parent.context), parent, false)
